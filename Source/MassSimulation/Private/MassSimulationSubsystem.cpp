...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MassSimulationSubsystem)
		...
		if (Context.WorldType != EWorldType::Inactive)
		{
			...
			if (UMassSimulationSubsystem* MassSimulationSubsystem = UWorld::GetSubsystem<UMassSimulationSubsystem>(World))
			{
				if (UE::Mass::Simulation::bSimulationTickingEnabled)
				{
					...
					if (MassSimulationSubsystem->HasCalledBeginPlay())
					{
...
void UMassSimulationSubsystem::RebuildTickPipeline()
{
	UMassEntitySettings* Settings = GetMutableDefault<UMassEntitySettings>();
	check(Settings);
	TConstArrayView<FMassProcessingPhaseConfig> ProcessingPhasesConfig = Settings->GetProcessingPhasesConfig();
	FString DependencyGraphFileName;

#if WITH_EDITOR
	const UWorld* World = GetWorld();
	if (World != nullptr && !Settings->DumpDependencyGraphFileName.IsEmpty())
	{
		...
		case EMassProcessingPhase::PrePhysics:
			{
				...
				check(SimSettings);
				if (UE::Mass::Simulation::bDoEntityCompaction && SimSettings->bEntityCompactionEnabled)
				{
					TRACE_CPUPROFILER_EVENT_SCOPE(DoEntityCompaction);
					check(EntityManager);
					EntityManager->DoEntityCompaction(SimSettings->DesiredEntityCompactionTimeSlicePerTick);
				}
			}
			break;
		default:
