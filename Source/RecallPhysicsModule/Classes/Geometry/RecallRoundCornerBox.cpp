...
#include "RecallRoundCornerBox.h"
...
public:
        ...
        inline void Get(TArray<FVector>& OutVertices, TArray<int32>& OutIndices)
        {
                OutVertices.Empty(vertices.size());
                for(const Vector3d& v : vertices)
                {
                        OutVertices.Add(FVector(v.x(), v.y(), v.z()));
                }
            
                OutIndices.Empty(indices.size() * 3);
                for(const Vector3i& idx : indices)
                {
                        OutIndices.Add(idx.x());
                        OutIndices.Add(idx.y());  
                        OutIndices.Add(idx.z());
                }
        }
...
namespace Recall::Geometry::Utils
{
   ...
   void CreateRoundCornerBox(int32 N, const FVector& Extents, float EdgeRadius,
                TArray<FVector>& OutVertices, TArray<int32>& OutIndices)
   {
           FRoundCornerBox RoundCornerBox;

           Vector3d Box(Extents.X, Extents.Y, Extents.Z);
           RoundCornerBox.Create(N, Box, EdgeRadius);
	        
           RoundCornerBox.Get(OutVertices, OutIndices);
   }
