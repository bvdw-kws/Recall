...
protected:
	 ...
	 * @param BuildContext if provided, will be used to store additional information for reporting back to the user
	 */
	MASSSPAWNER_API void GetCombinedTraits(TArray<UMassEntityTraitBase*>& OutTraits, FMassEntityTemplateBuildContext* BuildContext = nullptr) const;

	/** Looks for a trait of the indicated type, accepting all child classes as well, unless bExactMatch == true.
	 *  Walks the parent chain iteratively and detects cycles - logs an error and returns nullptr if a parent is reached twice. */
...
private:
	MASSSPAWNER_API void GetCombinedTraitsInternal(TArray<UMassEntityTraitBase*>& OutTraits, TArray<const UObject*>& Visited, FMassEntityTemplateBuildContext* BuildContext) const;
