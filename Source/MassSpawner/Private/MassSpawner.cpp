...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MassSpawner)
...
void AMassSpawner::OnSpawnDataGenerationFinished(TConstArrayView<FMassEntitySpawnDataGeneratorResult> Results, FMassSpawnDataGenerator* FinishedGenerator)
{
	...
	// @todo: this can be potentially expensive copy for the instanced structs, could there be a way to use move here instead?
...
void AMassSpawner::SpawnGeneratedEntities(TConstArrayView<FMassEntitySpawnDataGeneratorResult> Results)
{
	...
	for (FMassSpawnedEntityType& EntityType : EntityTypes)
	{
		if (UMassEntityConfigAsset* EntityConfig = EntityType.GetEntityConfig())
		{
			EntityConfig->AddTrait(UMassCreatedBySpawnerTrait::StaticClass());
		}
	}
#endif
	...
	UMassEntitySubsystem* EntitySubsystem = World->GetSubsystemChecked<UMassEntitySubsystem>();
...
#if WITH_EDITOR
				for (FMassEntityHandle Entity : SpawnedEntities.Entities)
				{
					if (FMassCreatedBySpawner* CreatedBy = EntityManager.GetFragmentDataPtr<FMassCreatedBySpawner>(Entity))
					{
						CreatedBy->Spawner = this;
					}
				}
#endif
			}
...
void AMassSpawner::DoDespawning()
{
	...
	if (SpawnerSystem == nullptr)
	{
		UE_LOGF(LogMassSpawner, Error, "UMassSpawnerSubsystem missing while trying to despawn entities");
		return;
	}
...
bool AMassSpawner::DespawnEntity(const FMassEntityHandle Entity)
{
	...
	if (SpawnerSystem == nullptr)
	{
		UE_LOGF(LogMassSpawner, Error, "UMassSpawnerSubsystem missing while trying to despawn a single entity");
		return false;
	}
